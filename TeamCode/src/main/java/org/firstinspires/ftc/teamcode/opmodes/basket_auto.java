...
public class basket_auto extends LinearOpMode {
    ...
    protected void clip(double cur_time) throws InterruptedException{
        traj1 = drive.actionBuilder(start)
                .splineToLinearHeading(clip,Math.toRadians(180))
                .build();
        bot.getBlockarm().set_grab(block_arm.Position.preclip);
        traj1.run(telemetryPacket);
    ...
    protected void pickup(double cur_time) throws InterruptedException{
        ...
        traj2 = drive.actionBuilder(traj1.)
                .lineToSplineHeading(pickup1)
                .back(5)
                .build();
        traj3 = drive.actionBuilder(traj3.end())
